...
export const retrieveNCGroup = async (site: string, ncGroup: string) => {
  const response = await api.post('/ncgroup-service/retrieve', { site, ncGroup });
...
export const createNCGroup = async (req: any) => {
  try {
    debugger
    const response = await api.post('/ncgroup-service/create', req);
    console.log('Create Response:', response);

    return response.data;
  } catch (error) {
    console.error('Error creating nc group:', error);
    throw error;
  }
};

export const updateNCGroup = async (req: any) => {
  try {
    const response = await api.post('/ncgroup-service/update', req);
    // console.log('Update Item Response:', response);

    return response.data;
  } catch (error) {
    console.error('Error updating nc group:', error);
    throw error;
  }
};
...
export const retrieveAllINCGroup = async (site: string,  ncGroup: string) => {
  const response = await api.post('/ncgroup-service/retrieveAll', { site, ncGroup });
  debugger
...
export const deleteNcGroup = async (site: string,  ncGroup: string, userId: string) => {
  const response = await api.post('/ncgroup-service/delete', {site, ncGroup, userId});
  // console.log(response,'All Item Hroup List Response');
  return response.data;
};

export const retrieveAvailableNCCodes = async (site: string,  ncGroup: string) => {
  const response = await api.post('/ncgroup-service/getAvailableNcCode', {site, ncGroup});
  // console.log(response,'All Item Hroup List Response');
  return response.data;
};
