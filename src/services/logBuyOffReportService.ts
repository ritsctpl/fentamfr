...
export const fetchtop50LineClearance = async (site: string) => {
  ...
  export const retrieveBuyOffTop50List = async (site: any) => {
    const response = await api.post('/buyoff-service/retrieveTop50', { ...site });
    return response.data;
  };

  export const retrieveAllBuyOffList = async (payload:object) => {
    const response = await api.post('buyoff-service/retrieveAll', { ...payload });
    return response.data;
  };
