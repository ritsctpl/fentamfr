...
import { fetchStart } from '@services/podServices';
...
const SignOffPlugin: React.FC<ChangeUnHoldStatusProps> = ({
  filterFormData,
  selectedRowData,
  setCall1,call1
}) => {
  ...
  const handleSubmit = async (values: any) => {
    ...
    const data = {
      ...
      defaultPhaseId: filterFormData.defaultPhaseId,
      defaultResource: filterFormData.defaultResource,
    ...
    const response = await fetchStart('processorderstate-service/signoff', data ,'signOff', data, filterFormData.defaultPhaseId);
    console.log(response, "response");
    if (response?.status === "success") {
      message.success(response?.processedSignoffBatches[0]?.message);
      setCall1(call1 + 1);
  } else {
