...
const StackedBarChart: React.FC<StackedBarChartProps> = ({ data, title, color }) => {
    ...
    const categoriesArray = Array.from(new Set(data.map(item => item[categoryKey])));
            ...
            if (reasonsKey) {
                ...
                valueKeys.forEach(valueKey => {
                    if (seriesMap[valueKey]) {
                        ...
                        if (tabkey === 'oee') {
                            seriesMap[`${valueKey}_remaining`].data[categoryIndex] = 100 - value;
                        }
                    }
                });
            }
    ...
    return (
        ...
        >
            <ReactECharts option={option} style={{ height: '300px' }} ref={echartsRef} />
                    ...
                    {open && (
                        <div style={{
                            ...
                            <div style={{ marginBottom: '20px' }}>
                                ...
                                <p>This stacked bar chart visualizes {title.toLowerCase()} metrics across different categories, allowing for easy comparison and trend analysis.</p>
                                <p>Each bar segment represents a distinct component of the {title.toLowerCase()} measurement, stacked to show the total contribution.</p>
                            </div>

                            <Title level={5}>Performance Thresholds</Title>
                            <div style={{ marginBottom: '20px' }}>
                                {tabkey && getSeriesColor(tabkey,color)?.map((color, index) => (
                                    <div key={index} style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
                                        <div style={{ width: '20px', height: '20px', backgroundColor: color, marginRight: '10px' }}></div>
                                        <span>{getSeriesColor(tabkey,color).meanings?.[index] || `Category ${index + 1}`}</span>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div style={{ marginBottom: '20px' }}>
                                <p><strong>Time Period:</strong> {categoriesArray[0]} to {categoriesArray[categoriesArray.length - 1]}</p>
