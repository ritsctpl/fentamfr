...
const SupervisionTracking: React.FC = () => {
  ...
  const  debounceHandleChange = debounce((newValue: any, key: number, field: keyof SupervisionTracking) => {
    ...
    if (field === 'validFrom' || field === 'validTo') {
      ...
      if (updatedItem && updatedItem.validFrom && updatedItem.validTo) {
        const validFrom = moment(updatedItem.validFrom);
        const validTo = moment(updatedItem.validTo);
  ...
  const handleDateChange = (date: moment.Moment | null, key: number, field: keyof SupervisionTracking) => {
    setIsAletered(true);
    if (date) {
    ...
    },
    {
      ...
      render: (_, record) => (
        <DatePicker
          ...
          value={record.validFrom ? moment(record.validFrom) : null}
    ...
    },
    {
      ...
      render: (_, record) => (
        <DatePicker
          showTime
          format="YYYY-MM-DD HH:mm:ss"
          value={record.validTo ? moment(record.validTo) : null}
