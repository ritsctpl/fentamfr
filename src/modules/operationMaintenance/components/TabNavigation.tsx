...
import DynamicForm from '@modules/operationMaintenance/components/DynamicForm';
import CertificateTab from '@modules/operationMaintenance/components/CertificateTab';
import SubStepTable from '@modules/operationMaintenance/components/SubStep';
import ActivityHookTable from '@modules/operationMaintenance/components/activityTab';
import CustomDataTable from '@modules/operationMaintenance/components/customData';
import styles from '@modules/operationMaintenance/styles/Tab.module.css';
import { useTranslation } from 'react-i18next';



const TabNav = ({ onCancel }) => {
  const { isEditing, erpShift, setErpShift, setFormData, formData, call, setCall } = useContext(OperationContext);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await updateOperation(site, userId, payload);
  ...
  const handleAdd = async () => {
    try {
      ...
      if (!formData.operation) {
        throw new Error('Operation fields cannot be empty.');
      }
      ...
      const res = await addOperation(site, userId, payload);
      ...
      console.error('Error:', error);
      message.error(error.message || 'An error occurred while adding the operation. Please check your input.');
    }
  };
