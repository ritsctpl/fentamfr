...
import { fetchUserWorkCenter } from '@services/shopOrderService';
...
const UserGroup: React.FC = () => {
  const { formData, setFormData } = useContext(UserContext); // Ensure setFormData is provided in context
  ...
  useEffect(() => {
    fetchData();
  }, [formData.user]); // Dependency on formData.user if needed
