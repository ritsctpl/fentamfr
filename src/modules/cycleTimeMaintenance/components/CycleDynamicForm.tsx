...
import { Form, Input, InputNumber, Modal, Table, TimePicker } from 'antd';
import { useTranslation } from 'react-i18next';
import { GrChapterAdd } from 'react-icons/gr';
import { parseCookies } from 'nookies';
import { fetchAllMaterial, fetchAllOperation, fetchAllResource, fetchAllWorkCenter, fetchTop50Material, fetchTop50Operation, fetchTop50Resource, fetchTop50WorkCenter } from '@services/cycleTimeService';
import { MaterialData, OperationData, ResourceData, ResourceTypeData, WorkCenterData } from '../types/cycleTimeTypes';
...
interface ResourceUpdateResponse {
    availableResourceType: Array<{
        resourceType: string;
    }>;
}

const CycleDynamicForm: React.FC<CycleDynamicFormProps> = ({ data, fields, onValuesChange }) => {
    const { t } = useTranslation();
    const [form] = Form.useForm();
    const { formData, setFormData, isFormDisabled, setIsFormDisabled } = useContext<any>(CycleTimeUseContext);
    ...
    const [cycleTimeEnabled, setCycleTimeEnabled] = useState(true); // Default cycleTime is enabled
    const [manufacturedTimeEnabled, setManufacturedTimeEnabled] = useState(false); // Default manufacturedTime is disabled
...
export default CycleDynamicForm;
