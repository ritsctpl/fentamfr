...
const NewLineChart: React.FC<LineChartProps> = ({ data, title,description }) => {
  ...
  const categoriesArray = Array.isArray(data) ? data.map(item => item.date) : [];
  const valueKeys = data && data.length > 0 ? Object.keys(data[0]).filter(key => key !== 'date') : [];
  ...
  const xAxisData = Array.isArray(data) ? data.map(item => item.date) : [];
    ...
    series: [
      {
        ...
        type: 'line',
        smooth: true,
        data: Array.isArray(data) ? data.map(item => item.downtime) : [],
        yAxisIndex: 0,
        itemStyle: {
      ...
      },
      {
        ...
        type: 'line',
        smooth: true,
        data: Array.isArray(data) ? data.map(item => item.output) : [],
        yAxisIndex: 1,
        itemStyle: {
