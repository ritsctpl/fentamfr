...
const FilterSettings: React.FC<SettingsProps> = ({ visible }) => {
  ...
  const handleTabChange = (value: string) => {
    ...
    if (!value) {
      ...
      setValue(ListValue.indexOf(value) );
    }
  ...
  const renderChart = () => {
    ...
    switch (selectedCategory) {
      ...
      case "oeeByComponent":
        ...
        //   data={}
        //   color={filterColors.oeeByProduct}
        // />
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
                ...
                setFilterColors((prev) => ({
                  ...
                  [selectedCategory]: {
                    ...
                    threshold: [
                      ...
                      Math.max(prev[selectedCategory]?.threshold?.[1] || value, prev[selectedCategory]?.threshold?.[2] || 20)
                    ],
                  },
                }));
              }}
                ...
                setFilterColors((prev) => ({
                  ...
                  [selectedCategory]: {
                    ...
                    threshold: [
                      ...
                      value,
                      Math.max(value, prev[selectedCategory]?.threshold?.[2] || value)
                    ],
                  },
                }));
              }}
          ...
          {/* <Space direction="vertical">
            <Slider
              min={0}
              max={100}
              value={filterColors[selectedCategory]?.threshold?.[2] || 0}
              onChange={(value) => {
                setFilterColors((prev) => ({
                  ...prev,
                  [selectedCategory]: {
                    ...prev[selectedCategory],
                    threshold: [
                      prev[selectedCategory]?.threshold?.[0] || 50,
                      prev[selectedCategory]?.threshold?.[1] || 85,
                      Math.max(prev[selectedCategory]?.threshold?.[1] || 85, value)
                    ],
                  },
                }));
              }}
              style={{ width: '100px' }}
            />
            <span>{filterColors[selectedCategory]?.threshold?.[2] || 0}</span>
          </Space> */}
        </div>
      </Space>}
    </div>
