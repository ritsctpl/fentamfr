...
const DocumentPreview: React.FC<DocumentPreviewProps> = ({ data, onDownload }) => {
    ...
    useEffect(() => {
        ...
        if (container) {
            container.addEventListener('downloadPDF', handleDownloadEvent);
        }

        return () => {
            if (container) {
                container.removeEventListener('downloadPDF', handleDownloadEvent);
            }
        };
    }, []);  // Empty dependency array since downloadAsPDF uses only ref
    ...
    const downloadAsPDF = async () => {
        ...
        try {
            ...
            const pageElements = pagesContainer.querySelectorAll('[data-page]');
            ...
            for (let i = 0; i < pageElements.length; i++) {
                ...
                // Add image to PDF (first page doesn't need addPage)
                if (i > 0) {
                    pdf.addPage();
                }
...
export { DocumentPreview as default, type DocumentPreviewProps }; 