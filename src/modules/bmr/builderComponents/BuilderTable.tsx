...
const BuilderTable: React.FC<any> = ({ isEditable = true, props }) => {
    const { title, onChange, type, metaData, componentId } = props;
    const { heading, table } = props.style || { heading: { titleAlign: 'left' }, table: {} };
    const [tableData, setTableData] = useState<any[]>(props.data || [{ key: '0' }]);
    const [originalData, setOriginalData] = useState<any[]>(props.data || [{ key: '0' }]);
    const [selectedRowKeys, setSelectedRowKeys] = useState<string[]>([]);
    const [hasChanges, setHasChanges] = useState(false);

    useEffect(() => {
        if (props.data && props.data.length > 0) {
            // Add key property to each row for table functionality
            const tableRows = props.data.map((row: any, index: number) => ({
                ...row,
                key: index.toString()
            }));
    ...
    const handleSave = () => {
        try {
            ...
            currentTemplate.components.main[componentIndex].config.data = tableData.map(row => {
