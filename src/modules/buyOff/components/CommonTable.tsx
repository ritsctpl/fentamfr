...
const CommonTable: React.FC<CommonTableProps> = ({ data, onRowSelect }) => {
  ...
  ? Object.keys(data[0]).map((column) => ({
      ...
      sorter: (a, b) => {
        ...
        // Handling boolean comparison
        if (typeof aValue == 'boolean' && typeof bValue == 'boolean') {
          return aValue == bValue ? 0 : aValue ? -1 : 1;
        }

        // For unsupported types, consider them equal
        return 0;
      },
      render: (value) => {
        // Handling boolean display
        if (typeof value == 'boolean') {
          return value ? "true" : "false"; // Change 'Yes'/'No' to any representation you prefer
        }
