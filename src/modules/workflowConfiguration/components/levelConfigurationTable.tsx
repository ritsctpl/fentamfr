...
const ListTable: React.FC = () => {
    ...
    const updateField = (id: string, key: keyof any, value: any) => {
        ...
        if (key === 'user') {
            // For user field, allow selective selection/deselection
        ...
        if (key === "finalApproval" || key === "action" || key === "user" || key === "from" || key === "to") {
            setPayloadData((prev) => ({
                ...prev,
                levelConfigurationList: updatedFields
            }));
    ...
    const handleLevelConfig = async (id, key, value) => {
      ...
      const handleUserDropdownClick = async (record: any) => {
        try {
          const cookies = parseCookies();
          const site = cookies?.site;

          // Get the current user role from the record
          const userGroup = record?.userRole;
          ...
          // Update the payload data with the fetched user list
          setPayloadData(prev => ({
            ...prev,
            userList: userList,
            userDropDownList: userDropDownList
          ...
          console.error("Error in retrieving user configuration: ", e);
          message.error(t('errorRetrievingUsers'));
        }
      };
    ...
    const baseColumns = [
        ...
        },
        {
            ...
            render: (user: string, record: any) => (
                <Select
                    ...
                    onClick={() => handleUserDropdownClick(record)}
                    style={{ width: '100%' }}
                >
        ...
        {
            title: <span>{t('from')} </span>,
            dataIndex: 'from',
            key: 'from',
            render: (from: string, record: any) => (
                // <Select
                //     value={from}
                //     defaultValue='Qa Review'
                //     onChange={(value) => updateField(record.id, 'from', value)}
                //     style={{ width: '100%' }}
                // >
                //     <Option value="Qa Review">Qa Review</Option>
                //     <Option value="Qc Review">Qc Review</Option>
                // </Select>
                 <Select defaultValue="" value={from} style={{ width: '100%' }}  onChange={(value) => updateField(record.id, 'from', value)}>
        ...
        {
            title: <span>{t('to')} </span>,
            dataIndex: 'to',
            key: 'to',
            render: (to: string, record: any) => (
                // <Select
                //     value={to}
                //     defaultValue='Qc Review'
                //     onChange={(value) => updateField(record.id, 'to', value)}
                //     style={{ width: '100%' }}
                // >
                //     <Option value="Qa Review">Qa Review</Option>
                //     <Option value="Qc Review">Qc Review</Option>
                // </Select>
                <Select defaultValue="" value={to} style={{ width: '100%' }}  onChange={(value) => updateField(record.id, 'to', value)}>
                {payloadData?.statesList?.map((state) => (
                  <Select.Option key={state.name} value={state.name}>
                    {state.name}
                  </Select.Option>
