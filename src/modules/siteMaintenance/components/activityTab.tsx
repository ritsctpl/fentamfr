...
const ActivityHookTable: React.FC = () => {
  ...
  const [dataSource, setDataSource] = useState<ActivityHook[]>(() => 
    (formData.activityHookLists || []).map((item, index) => ({
      id: index + 1,
  ...
  useEffect(() => {
    if (formData.activityHookLists) {
      const updatedDataSource = formData.activityHookLists.map((item, index) => ({
        id: index + 1,
        hookPoint: item.hookPoint,
        activity: item.activity,
        enabled: item.enabled,
  ...
  }, [formData.activityHookLists]);

  // Update formData when dataSource changes
  useEffect(() => {
    // Avoid updating formData if it hasn't changed
    if (JSON.stringify(dataSource) !== JSON.stringify(formData.activityHookLists)) {
      setFormData(prevFormData => ({
        ...prevFormData,
        activityHookLists: dataSource
      }));
    }
    ...
    },
    {
      ...
      render: (_, record) => (
        <Input
          ...
          onChange={e => handleChange(e.target.checked, record.id, 'enabled')}
        />
      ),
    },
    {
