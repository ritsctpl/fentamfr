...
const Home = ({ onCancel }) => {
  const { isEditing, erpShift, setFormData, formData, call, setCall, setIsFullScreen } = useContext(InventoryContext);
  ...
  const handleSave = async () => {
    ...
    if (!formData.inventoryId) {
      throw new Error('Inventory fields cannot be empty.');
    }
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      await mainForm.validateFields();
      if (!formData.inventoryId) {
        message.error('Inventory fields cannot be empty.');
        return
      }
  ...
  const fieldsToInclude = ['inventoryId',
    'description', 'item', 'version', 'receiveQty'
  ];
      ...
      <TabContext value={activeTab}>
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
          <TabList onChange={handleChange} aria-label="lab API tabs example">
            ...
            <Tab label={t('inventoryLocation')} value="2" />
          </TabList>
        </Box>
        <TabPanel value="1">
          <DynamicForm
        ...
        <TabPanel value="2"><InventoryLocation /></TabPanel>
      </TabContext>
