...
import { fetchResourceTypeAll, fetchResourceTypeTop50, RetriveResourceTypeSelectedRow } from '@services/resourceTypeServices';
import { ResourceTypeContext } from '@modules/resourceTypeMaintenance/hooks/ResourceTypeContext';
import ResourceTypeBody from '@modules/resourceTypeMaintenance/components/ResourceTypeBody';
import { defaultExtendedResourceTypeRequest, ExtendedResourceTypeRequest } from '../types/ResourceTypes';
...
const ResourceTypeMain: React.FC = () => {
  const cookies = parseCookies();
  const [site, setSite] = useState<string | null>(cookies.site);
  const [formData, setFormData] = useState<ExtendedResourceTypeRequest>(defaultExtendedResourceTypeRequest);
  ...
  useEffect(() => {
    const fetchResourceData = async () => {
      ...
      try {
        const item = await fetchResourceTypeTop50(site);
        setItemData(item);
  ...
  const handleTop50 = async () => {
    ...
    try {
      const response = await fetchResourceTypeTop50(site);
  ...
  const handleSearchClick = async (searchTerm: string) => {
    ...
    try {
      const AllBom = await fetchResourceTypeAll(site);
  ...
  const SelectRow = async (row) => {
    ...
    try {
      const rowData = await RetriveResourceTypeSelectedRow(site, row);
  ...
  return (
    <ResourceTypeContext.Provider value={{ formData, setFormData, setFormChange, formChange, activeTab, setActiveTab }}>
      <div className={styles.container}>
        <div className={styles.dataFieldNav}>
          <CommonAppBar
            ...
            username={username}
            site={null}
            appTitle={t("resourceTypeMaintenance")} onSiteChange={handleSiteChange} />
        </div>
        <div className={styles.dataFieldBody}>
          <div className={styles.dataFieldBodyContentsBottom}>
            <div className={`${styles.commonTableContainer} ${isAdding ? styles.shrink : ''}`}>
              ...
              button={
                <InstructionModal title="Resource Type Maintenance">
                  <UserInstructions />
                </InstructionModal>
              }
              />
              <div className={styles.dataFieldBodyContentsTop}>
                <Typography className={styles.dataLength}>
                  {t("resourceType")}({filteredData ? filteredData.length : 0})
                </Typography>
            ...
            <div className={`${styles.formContainer} ${isAdding ? `${styles.show} ${fullScreen ? styles.showFullScreen : styles.show}` : ''}`}>
              <ResourceTypeBody selected={selected} isAdding={isAdding} resetValue={resetValue} fullScreen={fullScreen}
                drag={drag} call={call} setCall={setCall} onClose={handleClose} setIsAdding={setIsAdding} setFullScreen={setFullScreen} />
            </div>
          </div>
        </div>
      </div></ResourceTypeContext.Provider>
  );
};

export default ResourceTypeMain;
