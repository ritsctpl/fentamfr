...
interface QcParameterTableProps {
  selectedIngredient: { qcParameters: []; ingredientId: string };
  setQcParameter: any;
  qcParameter: any;
}
...
const IngredientsTable: React.FC<QcParameterTableProps> = ({
  selectedIngredient,
  setQcParameter,
  qcParameter,
}) => {
  ...
  const handleInsert = () => {
    ...
    // Generate sequence number based on current table length
  ...
  const handleRemoveAll = () => {
    setIngredients(null);
    setQcParameter(null); // Clear qcParameter as well
  };

  const handleRemoveSelected = () => {
    const newIngredients = ingredients.filter(
      (item) => !selectedRowKeys.includes(item.ingredientId)
    );
  ...
  return (
    <div>
      ...
      >
        ...
        <Popconfirm
          title="Are you sure you want to remove all ingredients?"
        ...
        >
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item
                ...
                initialValue={selectedIngredient?.ingredientId}
              >
                <DynamicBrowse 
                  uiConfig={uiitem} 
                  initial={item} 
          ...
          <Row gutter={16}>
            <Col span={12}>
              ...
              >
                <Select options={batchUomOptions} />
              </Form.Item>
            </Col>
            <Col span={12}>
