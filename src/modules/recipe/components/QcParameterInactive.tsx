...
const QcParameterTable: React.FC<QcParameterTableProps> = ({ selectedIngredient, setQcParameter ,qcParameter}) => {
  ...
  const handleRemoveSelected = (selectedRowKeys: string[]) => {
  ...
  const rowSelection = {
    onChange: (selectedRowKeys: string[]) => {
      handleRemoveSelected(selectedRowKeys);
    },
  };
console.log(localQcParameters,qcParameter,"localQcParameters");
  ...
  return (
    <div>
      <Space style={{ marginBottom: 16, display: 'flex', justifyContent: 'end' }}>
        ...
        <Button onClick={handleSaveChanges}>{t('Save Changes')}</Button> {/* Save button to commit changes */}
      </Space>
      <Table
        bordered={true}
