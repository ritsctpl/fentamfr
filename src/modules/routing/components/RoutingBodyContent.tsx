...
import { RoutingData, defaultRoutingData } from '../types/routingTypes';
...
interface RoutingMaintenanceBodyProps {
    isAdding: boolean;
...
const RoutingMaintenanceBody: React.FC<RoutingMaintenanceBodyProps> = ({ call, setCall, 
    ...
    const { activeTab, setActiveTab, isStepModalVisible, setIsStepModalVisible, fullScreen, setFullScreen, isNextPage, setIsNextPage, payloadData,
         setPayloadData, username, setShowAlert, isAdding } = useContext<any>(RoutingContext);
    ...
    const handleSave = (oEvent) => {
        ...
        const updatedRoutingStepList = updatedRowData.routingStepList.map((step) => {
            debugger
            if (!step.previousStepId || !step.nextStepId) {
                // Apply your condition here, for example:
                console.log(`Step ID ${step.stepId} has no previous or next step ID defined.`);
                message.error(`Step ID ${step.stepId} has no previous or next step ID.`);
                saveFlag = false;
                return;
            ...
            if (step.stepType == "Operation") {
                if (!step.operation || !step.operationVersion) {
                    console.log(`Step ${step.stepId} has an empty or undefined operation/operationVersion.`);
                    message.error(`Step ${step.stepId} : Operation cannot be empty `);
                    saveFlag = false;
                    return;
                    // Handle the case where operation or operationVersion is "" or undefined or null
                }
            } else if (step.stepType == "Routing") {
                if (!step.routingBO.split(",")[1] || !step.routingBO.split(",")[2]) {
                    console.log(`Step ${step.stepId} has an empty or undefined routing/routingVersion.`);
                    message.error(`Step ${step.stepId} : Routing cannot be empty `);
                    // Handle the case where stepRouting or routingVersion is "" or undefined or null
                    saveFlag = false;
                    return;
                }
            }
        ...
        const oCreateRoute = async () => { // Rename the inner function to avoid recursion
    ...
    const handleConfirmDelete = () => {
        const deleteRoute = async () => { // Rename the inner function to avoid recursion
            ...
            try {
                const routing = selectedRowData.routing as string;
                const version = selectedRowData.version as string;
                const userId = username;

                const oDeleteRouting = await deleteRouting(site, routing, version, userId); // Assuming retrieveItem is an API call or a data fetch function
    ...
    const handleOpenCopyModal = () => {
        ...
        form.resetFields();
        form.setFieldsValue({
            routing: selectedRowData?.routing + "_COPY" || '',
            version: "",
            description: ''
        });
    };
    ...
    const handleConfirmCopy = () => {
        form
            ...
            .then((values) => {
                ...
                console.log('Routing copied:', values);

                const updatedRowData: any = {
                    ...payloadData, // Ensure this is correctly formatted
                    userId: username
                };
                updatedRowData.routing = values.routing;
                ...
                const copyReasonCode = async () => { // Rename the inner function to avoid recursion
                    ...
                    try {
                        ...
                        const copyResponse = await createRouting(updatedRowData);
                        console.log("Copy routing response: ", copyResponse);
    ...
    const columns = [
        { title: 'stepId', dataIndex: 'stepId', key: 'stepId', type: 'input' },
        { title: 'stepType', dataIndex: 'stepType', key: 'stepType', type: 'select' },
        { title: 'stepDescription', dataIndex: 'stepDescription', key: 'stepDescription', type: 'input' },
        { title: 'previousStepId', dataIndex: 'previousStepId', key: 'previousStepId', type: 'input' },
        { title: 'nextStepId', dataIndex: 'nextStepId', key: 'nextStepId', type: 'input' },
    ...
    const renderTabContent = () => {
        switch (activeTab) {
            case 0:
                return  (<div style={{ overflowY: 'auto', maxHeight: 'calc(100vh - 270px', }}>  <RoutingForm
                    schema={schema} />
                      </div>)
    ...
    return (
        ...
        <div className={styles.pageContainer}>
                        ...
                        <div className={styles.split}>
                            <div>
                                <p className={styles.headingtext}>
                                    {selectedRowData ? selectedRowData.routing : ""}
                                </p>
                                {selectedRowData && (
                                    <>
            ...
            <Modal
                ...
                centered
            >
                <p>{t("deleteMessage")}: <strong>{selectedRowData?.routing}</strong>?</p>
            </Modal>
            <Modal
                title={t("confirmCopy")}
            ...
            >
                <Form
                    ...
                    layout="vertical"
                    initialValues={{
                        routing: selectedRowData?.routing || '',
                        version: '',
                        description: ''
                    }}
                >
                    <Form.Item
                        name="routing"
                        label={t("routing")}
                        rules={[{ required: true, message: 'Please enter the routing' }]}
                    >
                        <Input placeholder="" onChange={(e) => handleFieldChange(e, 'routing')} />
                    </Form.Item>
...
export default RoutingMaintenanceBody;