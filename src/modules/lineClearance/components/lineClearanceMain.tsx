...
import LineClearanceBar from "./lineClearanceBar";
import LineClearanceBody from "./lineClearanceBody";
import { lineClearanceContext } from "../hooks/lineClearanceContext";
import { ChecklistTemplate, defaultLineClearanceTemplate } from "../types/lineClearanceTypes";
import { fetchLineClearanceTop50, fetchLineClearanceAll,RetriveLineClearanceSelectedRow } from "@services/lineClearanceService";
...
const LineClearanceMain: React.FC = () => {
  ...
  useEffect(() => {
    const fetchResourceData = async () => {
      ...
      try {
        const item = await fetchLineClearanceTop50(site);
        setItemData(item);
  ...
  const handleTop50 = async () => {
    ...
    try {
      const response = await fetchLineClearanceTop50(site);
  ...
  const handleSearchClick = async (searchTerm: string) => {
    ...
    const site = cookies.site;
    try {
      const lineClearanceAll =
        await fetchLineClearanceAll(site);
      const lowercasedTerm = searchTerm.toLowerCase();
      let filtered: React.SetStateAction<DataRow[]>;
      if (lowercasedTerm) {
        filtered = lineClearanceAll?.filter(
  ...
  const SelectRow = async (row) => {
    ...
    try {
      const rowData = await RetriveLineClearanceSelectedRow(
        site,
        row
      );
      ...
      <div className={styles.container}>
        <div className={styles.dataFieldNav}>
          <CommonAppBar
            ...
            username={username}
            site={null}
            appTitle={t("lineClearanceMaintenance")}
            ...
            >
              ...
              <div className={styles.dataFieldBodyContentsTop}>
                <Typography className={styles.dataLength}>
                  {t("lineClearance")}({filteredData ? filteredData.length : 0}
                  )
                </Typography>
...
export default LineClearanceMain;
