...
const DynamicTable: React.FC<DynamicTableProps> = ({
  ...
  const handleCheckboxChange = (key: string, checked: boolean, record: any) => {
    setSelectedRowData(prevData => {
      if (checked) {
        if (!prevData.some(item => item.key === record.key)) {
  ...
  const handleRowClick = (record) => {
    const isChecked = selectedRowData.some(item => item.key === record.key);
    handleCheckboxChange(record.key, !isChecked, record);
  };
  ...
  const transformData = (data: { columnLists: any[] }[]): { column: ColumnsType<DataRow>; dataSource: DataRow[] } => {
    ...
    const column: ColumnsType<DataRow> = [
      {
        ...
        render: (_: any, record: DataRow) => (
          <Checkbox
            checked={selectedRowData.some(selected => selected.key === record.key)}
            onChange={(e) => handleCheckboxChange(record.key, e.target.checked, record)}
          />
        ),
      },
      ...columnOrder
        ...
        .map((item: any) => {
          ...
          return {
            title: item.dataField === 'batchNo' ? t('batchNo') : t(item.dataField),
            // title: podCategoryType === 'processOrder' ? t('batchNo') : t(item.dataField),
            dataIndex: item.dataField,
            ellipsis:true,
            key: item.dataField,
            width: columnConfig?.width && columnConfig.width !== "" ? parseInt(columnConfig.width) : 'auto',
    ...
    const dataSource: DataRow[] = data
      .map((item, index) => ({
        key: `${item.columnLists.find(col => col.dataField === 'batchNo')?.dataAttribute}_${item.columnLists.find(col => col.dataField === 'orderNumber')?.dataAttribute}`,
        ...item.columnLists.reduce((acc: any, col: any) => {
          acc[col.dataField] = col.dataField === 'Status' && col.dataAttribute.toLowerCase() === 'active'
            ? '🟢 Active'
            : col.dataField === 'Status' && col.dataAttribute === 'In Queue'
              ? '⏳ In Queue'
              : col.dataField === 'Status' && col.dataAttribute.toLowerCase() === 'hold'
                ? '🛑 Hold'
                : col.dataAttribute;
          return acc;
        }, {}),
      ...
      .sort((a, b) => {
        ...
        if (a.Status !== '🛑 Hold' && b.Status === '🛑 Hold') return 1;
        if (a.Status === '⏳ In Queue' && b.Status !== '⏳ In Queue') return -1;
        if (a.Status !== '⏳ In Queue' && b.Status === '⏳ In Queue') return 1;
        return 0;
      });
    return { column, dataSource };
