...
import { fetchSchedulerAll } from '@services/scheduler';
...
const DataFieldCommonBar: React.FC<DataFieldCommonBarProps> = ({ onSearch, setData }) => {
  ...
  const handleItemFetch = async () => {
    ...
    if(searchTerm!==""){
      try {
         const siteTop50List = await fetchSchedulerAll(site);
      ...
      try {
        const shopOrderData = await fetchSchedulerAll(site);
    ...
    <div className={styles.dataField}>
      <div className={styles.datafieldNav}>
        ...
        <div className={styles.goButton}>
          <Button onClick={handleItemFetchGo} className={styles.blueButton}>
            {t('go')}
          </Button>
        </div>
      </div>
