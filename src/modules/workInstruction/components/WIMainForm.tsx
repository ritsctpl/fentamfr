...
const WIMainForm: React.FC<DynamicFormProps> = ({
  ...
  const { payloadData, setPayloadData, schema, setShowAlert, rowSelectedData, setRowSelectedData } = useContext<any>(WorkInstructionContext);
  ...
  const handleInputChange = (value: string, field: FormField) => {
    ...
    if (field.type === 'number') {
      // Restrict to numbers only and prevent 'e' character
    ...
    // Update the form field with the transformed value only if newValue is not empty
    if (newValue !== "") {
      form.setFieldsValue({ [field.name]: newValue });
    } else {
      form.setFieldsValue({ [field.name]: "" }); // Set to empty if the value is invalid
    }
  ...
  const certificationTableColumns = [
  ...
  return (
    <>
      ...
      >
        <Table
          columns={certificationTableColumns}
