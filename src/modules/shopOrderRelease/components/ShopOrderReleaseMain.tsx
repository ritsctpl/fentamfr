...
import ShopOrderReleaseBar from '@modules/shopOrderRelease/components/ShopOrderReleaseBar';
import ShopOrderReleaseBody from '@modules/shopOrderRelease/components/ShopOrderReleaseBody';
import { fetchShopReleaseAll, fetchShopAllData, fetchShopTop50 } from '@services/shopOrderService';
import { defaultShopOrderReleaseTypesRequest } from '../types/ShopOrderReleaseTypes';
...
const ShopOrderReleaseMain: React.FC = () => {
  const cookies = parseCookies();
  const [site, setSite] = useState<string | null>(cookies.site);
  const [formData, setFormData] = useState<any>(defaultShopOrderReleaseTypesRequest);
  ...
  useEffect(() => {
    const fetchShopOrderRelease = async () => {
      ...
      try {
        const item = await fetchShopTop50(site);
        setItemData(item);
  ...
  const handleSearchClick = async (searchTerm: string) => {
    ...
    try {
      const AllShopOrder = await fetchShopAllData(site);
      const lowercasedTerm = searchTerm.toLowerCase();
      let filtered;

      if (lowercasedTerm) {
        filtered = AllShopOrder.filter(row =>
  ...
  const SelectRow = async (row) => {
    ...
    try {
      ...
      const rowData = await fetchShopReleaseAll(site, payload);
  ...
  return (
    <ShopOrderReleaseContext.Provider value={{ formData, setFormData, setFormChange, formChange }}>
      <div className={styles.container}>
        <div className={styles.dataFieldNav}>
          <CommonAppBar
            ...
            username={username}
            site={null}
            appTitle={t("shopOrderRelease")} onSiteChange={handleSiteChange} />
        </div>
        <div className={styles.dataFieldBody}>
          <div className={styles.dataFieldBodyContentsBottom}>
            <div className={`${styles.commonTableContainer} ${isAdding ? styles.shrink : ''}`}>
              <ShopOrderReleaseBar handleSearchClicks={handleSearchClick} handleTop50={handleTop50} />
              <div className={styles.dataFieldBodyContentsTop}>
                <Typography className={styles.dataLength}>
                  {t("shopOrder")}({filteredData ? filteredData.length : 0})
                </Typography>
              </div>
            ...
            <div className={`${styles.formContainer} ${isAdding ? `${styles.show} ${fullScreen ? styles.showFullScreen : styles.show}` : ''}`}>
              <ShopOrderReleaseBody selected={selected} isAdding={isAdding} resetValue={resetValue} fullScreen={fullScreen}
                drag={drag} call={call} setCall={setCall} onClose={handleClose} setIsAdding={setIsAdding} setFullScreen={setFullScreen} />
            </div>
          </div>
        </div>
      </div></ShopOrderReleaseContext.Provider>
  );
};

export default ShopOrderReleaseMain;
