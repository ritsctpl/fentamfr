...
const StackedBarChartpopup: React.FC<StackedBarChartProps> = ({
  data,
  title,
  color,
  style,
  ...
  // Check if "name" field exists in the data and prioritize it
  const nameKey = keys.includes("name") ? "name" : null;
  ...
  // Use nameKey if it exists, otherwise fall back to other possible category keys
  const categoryKey =
    nameKey ||
      ...
      formatter: function (params) {
        ...
        params.forEach((param) => {
          if (!param.seriesName.includes("_remaining")) {
            ...
            result += `${marker}${param.seriesName}: ${Number(
              param.value
            ).toFixed(2)}%<br/>`;
          }
        });
...
export default StackedBarChartpopup;
