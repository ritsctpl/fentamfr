...
import ItemGroupMaintenanceForm from './ItemGroupForm';
import GroupMember from './GroupMember';


interface ItemGroupMaintenanceBodyProps {
    isAdding: boolean;
...
const ItemGroupMaintenanceBody: React.FC<ItemGroupMaintenanceBodyProps> = ({ call, setCall,
    isAdding, selectedRowData, onClose, itemRowData, setFullScreen, username, setAddClick, fullScreen, rowSelectedData}) => {
    const { payloadData, setPayloadData, setShowAlert, activeTab, setActiveTab  } = useContext<any>(ItemGroupContext);
    ...
    const handleSave = (oEvent) => {
        ...
        if (payloadData.itemGroup == undefined || payloadData.itemGroup == null || payloadData.itemGroup == "") {
            message.error("Item Group cannot be empty");
            flagToSave = false;
            return;
        }

        const oCreateItemGroup = async () => { // Rename the inner function to avoid recursion
            try {
                if (oEvent.currentTarget.innerText == "Create" || oEvent.currentTarget.innerText == "बनाएं"
                    ...
                    const createResponse = await createItemGroup(updatedRowData);
                    ...
                    const updateResponse = await updateItemGroup(updatedRowData);
    ...
    const handleConfirmDelete = () => {
        const oDeleteItemGroup = async () => {
            const cookies = parseCookies();
            const site = cookies.site;

            try {
                const itemGroup: string = selectedRowData.itemGroup;
                const userId = username;
                debugger
                const response = await deleteItemGroup(site, itemGroup, userId);
                if (!response.errorCode) {
                    ...
                    console.log("Deleted item group: ", response);
                }
            } catch (error) {
    ...
    const handleConfirmCopy = () => {
        form
            ...
            .then((values) => {
                ...
                if (updatedRowData.itemGroup == null || updatedRowData.itemGroup == "" || updatedRowData.itemGroup == undefined) {
                    message.error("Item Group cannot be empty");
                    return;
                }

                const oCopyItemGroup = async () => { // Rename the inner function to avoid recursion

                    try {

                        const copyResponse = await createItemGroup(updatedRowData);
                        console.log("Copy Item Group response: ", copyResponse);
                        ...
                        console.error('Error copying item group:', error);
                    }
                };
    ...
    const renderTabContent = () => {
        switch (activeTab) {
            case 0:
                return <ItemGroupMaintenanceForm rowSelectedData={rowSelectedData} onChange={function (values: Record<string, any>): void {
            ...
            case 1:
                return (
                    <GroupMember setActiveTab={setActiveTab} />
    ...
    return (
        <div className={styles.pageContainer}>
                        ...
                        <div className={styles.split}>
                            <div>
                                <p className={styles.headingtext}>
                                    {selectedRowData ? selectedRowData.itemGroup : t('createItemGroup')}
                                </p>
                                {selectedRowData && (
                                    <>
            ...
            <Modal
                ...
                centered
            >
                <p>{t("itemGroupDeleteMessage")}: <strong>{selectedRowData?.itemGroup}</strong>?</p>
            </Modal>
            <Modal
                title={t("confirmCopy")}
...
export default ItemGroupMaintenanceBody;