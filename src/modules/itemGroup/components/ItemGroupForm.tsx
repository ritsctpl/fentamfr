...
interface ItemGroupMaintenanceFormProps {
...
const ItemGroupMaintenanceForm: React.FC<ItemGroupMaintenanceFormProps> = ({
  onChange,
  layout = 'horizontal',
  labelCol = { span: 8 },
  wrapperCol = { span: 12 },
  ...
  const { payloadData, setPayloadData, schemaData, addClickCount, mainSchema, showAlert, setShowAlert } = useContext<any>(ItemGroupContext);
  ...
  const openModal = async (fieldName: string) => {
    try {
      ...
      const toBeChanged = 'revision';
      const toBeChangedTo = 'bomVersion';
      const data = formattedBomList;
        ...
        return (
          ...
          >
            <Input
              ...
              onChange={e =>
                form.setFieldsValue({
                  [field.name]: handleNumericInput(e.target.value, field),
                })
              }
            />
          </Form.Item>
...
export default ItemGroupMaintenanceForm;