...
import NoDataScreen from "@modules/overallDashBoard/components/NoData";
...
const MachineTimeLine: React.FC<MachineTimeLineProps> = ({ shiftName, shiftData, date, workcenter, eventType }) => {
  ...
  const {selectedResources} = useFilterContext();
  ...
  if ((shiftData && typeof shiftData === 'object' && !shiftData.hasOwnProperty('shift_start_time')) || shiftData == undefined || shiftData == null) {
    return (
      // <Card
  ...
  return (
    // <Card size="small" title={"Machine Timeline"} >
      <Row gutter={24}>
      <Col span={24}>
          <TimelineContainer>
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <div style={{ width: '100px', marginRight: '4px' }}>
                <Text style={{ fontSize: '11px' }}>{selectedResources?.current}</Text>
              </div>
              <TimelineBar>
