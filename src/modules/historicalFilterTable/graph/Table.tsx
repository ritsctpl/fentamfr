  ...
  const columns: ColumnsType<Record<string, any>> = orderedColumns.map(
    (key) => ({
      ...
      render: (value: any, record: any) => {
        if (record.type === "summary" || record.data_type === "summary") {
          if (
            key === "interval_start_date_time" ||
              ...
              children: (
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontWeight: "600", fontSize: "13px" }}>
                    {`${formatDateTime(
                      record.interval_start_datetime
                    )} - ${formatDateTime(record.interval_end_datetime)}`}
                  </div>
          ...
          // For other columns in summary row
          return {
            children: value,
            props: {
              style: {
      ...
      width: (() => {
        if (
          ...
          return 120; // Reduced width for date-time columns
        }
        if (key.toLowerCase().includes("qty")) {
          return 80; // Fixed width for quantity columns
        }
        if (key.toLowerCase().includes("target")) {
          return 100; // Width for target columns
        }
        return undefined; // Let other columns auto-adjust
      })(),
      ellipsis: true,
    })
  );
    ...
    >
      ...
      {isDataEmpty ? (
        ...
        <Table
          ...
          scroll={{ y: "calc(100vh - 600px)", x: 800 }}
          pagination={false}
          bordered
          size="small"
